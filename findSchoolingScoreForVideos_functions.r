...
fillGapsBetweenSchooling <- function(isSchooling)
{
...
calculateSchoolingScoreForThisVideo2 <- function(fileName, firstFrame=1, lastFrame=9000, threshForCloseness=125, threshForTimeOfCloseNessInEvery30 = 15, threshForSpeed = 25)
{
...
#################calculating the speed of the fish (pixel/second)###################
  ...
  V = sqrt(diff(fishXMedian)^2 + diff(fishYMedian)^2);
  V = c(V,V[length(V)]);
  FishIsNotMovingSeconds = which(V<threshForSpeed)
...
##################################
  ...
  isSchooling = filledGapsSchooling(isSchooling);
